...
class PolarLoopChannelHandler(
    ...
    companion object {
        private const val METHOD_CHANNEL = "com.example.mobile_pulse/polar_loop/methods"
        private const val EVENT_CHANNEL = "com.example.mobile_pulse/polar_loop/events"
    ...
    private fun launchMethod(
        ...
        scope.launch {
            try {
                ...
                emitError(
                    scope = "method",
                    message = error.message ?: "Polar Loop operation failed",
                    code = error.javaClass.simpleName,
                )
                mainHandler.post {
                    result.error(
                        error.javaClass.simpleName,
                        error.message ?: "Polar Loop operation failed",
                        null,
                    )
                }
            }
