            ...
            children: [
                    ...
                    for (final entry in state.recordings) ...[
                      _RecordingCard(
                        ...
                        onExport: isConnected
                            ? () => notifier.exportOfflineRecord(entry.path)
                            : null,
                        onDelete: isConnected && entry.exportConfirmed
                            ? () => notifier.deleteOfflineRecord(entry.path)
                            : null,
                      ),
                      const SizedBox(height: 12),
                    ],
              ...
              _SectionCard(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  _ExportedRecordPanel(exportsByPath: state.exportsByPath),
                ],
              ),
            ],
          ),
...
class _ExportedRecordPanel extends StatelessWidget {
  ...
  const _ExportedRecordPanel({required this.exportsByPath});
