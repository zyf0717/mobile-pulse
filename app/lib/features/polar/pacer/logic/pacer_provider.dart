...
class PacerNotifier extends Notifier<PacerState> {
  ...
  PacerState build() {
    ...
    final errorSub = pacer.errorStream.listen((error) {
      if (!error.isConnectionError) {
        switch (error.stream) {
          case 'acc':
            state = state.copyWith(accRelayStatus: RelayPushStatus.error);
            break;
          case 'ppi':
            state = state.copyWith(ppiRelayStatus: RelayPushStatus.error);
            break;
        }
      }
  ...
  Future<void> _startPacerRelay() async {
    ...
    await _startStream(
      start: pacer.startAccStreaming,
      stopRelay: accRelay.stop,
      statusUpdate: () =>
          state = state.copyWith(accRelayStatus: RelayPushStatus.error),
      messagePrefix: 'ACC',
    );
    await _startStream(
      start: pacer.startPpiStreaming,
      stopRelay: ppiRelay.stop,
      statusUpdate: () =>
          state = state.copyWith(ppiRelayStatus: RelayPushStatus.error),
      messagePrefix: 'PPI',
    );
  ...
  Future<void> _stopPacerRelay() async {
    ...
    ppiRelay.stop();
    final pacer = ref.read(polarPacerServiceProvider);
    await pacer.stopAccStreaming();
    await pacer.stopPpiStreaming();
    state = state.copyWith(
      pacerRelayActive: false,
      hrRelayStatus: RelayPushStatus.idle,
      accRelayStatus: RelayPushStatus.idle,
  ...
  Future<void> _startStream({
    ...
    required void Function() stopRelay,
    required void Function() statusUpdate,
